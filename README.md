...
## Запуск проекта
1. Необходимо создать на компьютере базу данных Postgres (`POSTGRES_DATABASE`).
2. Изменить значения переменных окружения в файле .env на необходимые:
    ...
    POSTGRES_PORT=5432
    POSTGRES_DATABASE=coin_market
    
    JWT_KEY=supersecuresecret123
   ```
...
## Тесты
Все тесты написаны в папке /internal/tests
- integration - интеграционные
- unit - юнит  
> Общее покрытие кода тестами **> 60%**

## Спорные моменты
- Установил для JWT `expirationTime := time.Now().Add(24 * time.Hour)`
- Не использовал uuid для пользователя, чтобы в некоторых случаях не совершать дополнительную операцию поиска
- Использовал bcrypt для шифрования пароля, что замедляет работу, но добавляет безопасности
- Вынес предметы в отдельную таблицу, что замедляет работу, но позволяет расширять магазин мерча
